...
Elm.Visual.make = function (_elm) {
   ...
   freqdata) {
      return function () {
         ...
         0 - w / 2.0,
         h / -2.0))(A2($List.indexedMap,
         draw,
         freqdata));
      }();
   });
   var soundUrl = Elm.Native.Port.make(_elm).inboundSignal("soundUrl",
   "Maybe.Maybe String",
   function (v) {
   ...
   var slidersInput = A4($Signal.map3,
   ...
   var playButtonInput = $Signal.mailbox(controlState.playing);
   var playButtonCount = A3($Signal.foldp,
   F2(function (_v2,total) {
      return function () {
         return total + 1;
      }();
   }),
   0,
   playButtonInput.signal);
   var urlFieldInput = $Signal.mailbox(controlState.url);
   var controlInput = A3($Signal.map2,
   F2(function (v0,v1) {
      return {ctor: "_Tuple2"
             ,_0: v0
             ,_1: v1};
   ...
   state) {
      return function () {
         var btn = A2($Graphics$Input.button,
         A2($Signal.message,
         playButtonInput.address,
         $Basics.not(state.playing)),
         state.playing ? "Pause" : "Play");
         var url = $Graphics$Element.width($Basics.round(w / 2) - $Graphics$Element.widthOf(btn))(A4($Graphics$Input$Field.field,
         $Graphics$Input$Field.defaultStyle,
         $Signal.message(urlFieldInput.address),
            ...
            switch (_v4._0.ctor)
              {case "_Tuple2":
                 return function () {
                      ...
                      var currentVal = $WebAudio.getValue(_v4._0._1.gain);
   ...
   state) {
      ...
      function (_v8) {
         return function () {
            switch (_v8.ctor)
            {case "_Tuple2":
               return _U.eq(_v8._0,idx);}
            _U.badCase($moduleName,
            "on line 62, column 47 to 55");
         }();
      },
      state.selected));
   });
      ...
      return function () {
         ...
         return function () {
            ...
            {case "Just": return x._0;
               case "Nothing":
               return $Debug.crash(err);}
            _U.badCase($moduleName,
            "between lines 20 and 22");
         }();
      }();
   });
   var headOrDie = doOrDie($List.head);
   var tailOrDie = doOrDie($List.tail);
   var filters = function () {
      ...
      analyser,
      0,
      0)($WebAudio.setFilterType($WebAudio.HighShelf)(A2(headOrDie,
      rlst,
      "no filters")));
   ...
   var updateTrack = function ($) {
      return $WebAudio.playMediaElement($Maybe.withDefault(mediaStream)($Maybe.map(function (url) {
   ...
   var updateControls = F2(function (_v17,
   state) {
      return function () {
         switch (_v17.ctor)
         {case "_Tuple2":
   ...
   controlState,
   controlInput);
   var fetchSoundUrl = Elm.Native.Port.make(_elm).outboundSignal("fetchSoundUrl",
   function (v) {
      return v;
   },
   A2($Signal._op["<~"],
   function (_v21) {
      return function () {
         return _v21.url.string;
      }();
   },
   A3($Signal.filter,
   function (_v23) {
      return function () {
         return _v23.loadTrack;
      }();
   },
   controlState,
   mainControls)));
   var selectSlider = F3(function (_v25,
   _v26,
   state) {
      return function () {
         switch (_v26.ctor)
                 ...
                 {case "_Tuple2":
                    return function () {
                         ...
                         var $ = {ctor: "_Tuple2"
                                 ...
                                 ,_1: $Basics.toFloat(_v25._1)},
                         w = $._0,
                         h = $._1;
                         var x = $Basics.toFloat(_v26._0) - w / 2.0 + state.move * 5.0;
                         var y = h / 4.0 - $Basics.toFloat(_v26._1);
                            ...
                            return function () {
                               ...
                               {case "_Tuple2":
                                  return A4(handleHitTest,
                                    x - ($Basics.toFloat(_v33._0) + 0.5) * state.move,
                                    y,
                                    state.scale,
                                    _v33._1);}
                               _U.badCase($moduleName,
                               "on line 101, column 39 to 105");
                            }();
                         },
                         lst);
                         var selected = $List.head(filtered);
   ...
   var updateSliders = F2(function (_v37,
   state) {
      return function () {
         switch (_v37.ctor)
         {case "_Tuple3":
            return !_U.eq(_v37._0,
              ...
              _v37._1,
              _v37._2,
              state) : _v37._1 ? state.dragging ? A2(updateSelectedSlider,
              _v37._2,
   ...
   var updateSlidersVisual = F4(function (_v42,
   isdown,
   pos,
   state) {
      return function () {
         ...
         _U.badCase($moduleName,
         "between lines 78 and 84");
      }();
   });
   var mainSliders = A2($Signal.filter,
   function (m) {
      return m.changes;
   },
      ...
      return function () {
         return function () {
            ...
            {case "_Tuple2":
               return function () {
                    var $ = {ctor: "_Tuple2"
                            ,_0: $Basics.toFloat(_v46._0)
                            ,_1: $Basics.toFloat(_v46._1)},
                    w = $._0,
                    h = $._1;
                    var halfw = w / 2.0;
                    var halfh = h / 2.0;
